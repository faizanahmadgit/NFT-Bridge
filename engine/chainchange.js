...
export async function polyChain() {
    try {
        await ethereum.request({
            method: 'wallet_switchEthereumChain',
            params: [{ chainId: '0x89' }],      // chainid 137 for polygon mainnet
        });
    } catch (switchError) {
        if (switchError.code === 4902) {
            try {
...
export async function hardChain() {
    try {
        await ethereum.request({
            method: 'wallet_switchEthereumChain',
            params: [{ chainId: '0x7A69' }], //chainID 31337
        });
    } catch (switchError) {
        if (switchError.code === 4902) {
            try {
...
export async function bscTest() {
    try {
        await ethereum.request({
            method: 'wallet_switchEthereumChain',
            params: [{ chainId: '0x61' }],        //chainID  97 for binance testnet
        });
    } catch (switchError) {
        if (switchError.code === 4902) {
            try {
...
export async function ethTest() {
 ...
 //// for Mumbai Testnet of polygon network
export async function polyTest() {
    try {
        await ethereum.request({
            method: 'wallet_switchEthereumChain',
            params: [{ chainId: '0x13881' }],     //chainId 80001
        });
    } catch (switchError) {
        if (switchError.code === 4902) {
            try {
